...
const columns = [
  { title: '风险类型', key: 'type' },
  { title: '风险等级', key: 'level', render: (row: any) => {
    const types: Record<string, 'success' | 'warning' | 'error'> = { '低': 'success', '中': 'warning', '高': 'error' }
    return <n-tag type={types[row.level]}>{row.level}</n-tag>
  }},
